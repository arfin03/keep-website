...
/**
 * Render leaderboard.
 * @param {string} type - 'waifu' | 'husband' | 'charms' | '' (all)
 * @param {Event|null} ev
 */
async function renderTop(type = 'waifu', ev = null) {
  ...
  data.items.forEach((item, index) => {
    ...
    // avatar handling: prefer item.avatar (server should already avoid picsum). fallback to DEFAULT_AVATAR
    let avatar = item.avatar || item.photo_url || null;
    if (!avatar || String(avatar).trim() === '' || avatar.includes('picsum.photos')) {
      avatar = DEFAULT_AVATAR;
    }

    const name = item.name || item.firstname || 'Traveler';
    ...
    html += `
      <div class="glass-card" style="display:flex; align-items:center; gap:12px; padding:12px;">
        ...
        <img src="${escapeAttr(avatar)}" onerror="this.onerror=null;this.src='${DEFAULT_AVATAR}';" 
             style="width:44px; height:44px; border-radius:50%; object-fit:cover; border:2px solid rgba(255,255,255,0.12);">
        <div style="flex:1;">
